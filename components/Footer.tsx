import React from 'react';

export const Footer: React.FC = () => {
    return (
        <footer className="w-full bg-black text-white py-6 border-t border-gray-900 mt-auto">
            <div className="max-w-screen-xl mx-auto px-4 md:px-8">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    <div className="flex flex-col md:flex-row items-center gap-2 md:gap-6">
                        <h3 className="text-lg font-black italic tracking-tighter">IPAP DEPT.</h3>
                        <p className="text-gray-500 text-[10px] uppercase tracking-widest hidden md:block">
                            Premium Headwear Culture
                        </p>
                    </div>

                    <div className="flex items-center gap-6">
                        <p className="text-[10px] text-gray-500 font-mono uppercase tracking-tight">
                            &copy; {new Date().getFullYear()} All rights reserved.
                        </p>
                        <a href="https://facebook.com/profile.php?id=61581559561803" target="_blank" rel="noreferrer" className="text-gray-400 hover:text-white transition-colors">
                            <span className="sr-only">Facebook</span>
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
                        </a>
                    </div>
                </div>
            </div>
        </footer>
    );
};
